...
class RocksetAdapter(BaseAdapter):
    ...
    def get_rows_different_sql(
        ...
        names: List[str]
        skip_cmp_columns: Set[str] = {'_event_time', '_id', '_meta'} # columns generated by Rockset
        if column_names is None:
            columns = self.get_columns_in_relation(relation_a)
            names = sorted((self.quote(c.name) for c in columns if c.name not in skip_cmp_columns))
        else:
            names = sorted((self.quote(n) for n in column_names if n not in skip_cmp_columns))
